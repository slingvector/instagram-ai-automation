...
class BulkPosterStateMachine:
    ...
    def _process_single_reel(self, reel: Dict[str, Any], tag: str, args=None) -> int:
        ...
        try:
            ...
            # State 5: MEDIA_PROCESSED -> SYNCED_TO_DRIVE (Manual Detour)
            if state == ReelState.MEDIA_PROCESSED and args and args.manual:
                if not reel.get('drive_folder_id'):
                    logger.info(f"{tag} 📁 Syncing to Google Drive (Manual Mode)...")
                    ...
                    return 0 # No wait time needed for manual sync
                else:
                    logger.info(f"{tag} Skipping Drive Sync: Already synced.")
                    return 0

            # State 6: MEDIA_PROCESSED -> POSTING -> POSTED (Automated Path)
            if state == ReelState.MEDIA_PROCESSED:
                self.state_manager.update_state(url, ReelState.POSTING)
                if not args.skip_preflight:
...
def run():
    ...
    parser.add_argument("--manual", action="store_true", help="Prepare for manual posting via Google Drive")
